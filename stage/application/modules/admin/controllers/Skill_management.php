...
class Skill_management extends MX_Controller {
    ...
    public function all_requested_skill() {
        ...
        $data['query'] = $this->common_model->RetriveRecordByWhere('requested_skills', array());
    ...
    public function approve_skill() {
        $id = $this->input->post("id");
        $data = $this->common_model->RetriveRecordByWhereRow('requested_skills', array('id' => $id));
        $getData = array(
            'skill_for' => $data['skill_for'],
            'name' => $data['name'],
        );
        $this->common_model->AddRecord($getData, 'skills');
        $this->common_model->Delete('requested_skills', $id, 'id');
        $message = '<div class="alert alert-success"><p>Successfully added.</p></div>';
        $this->session->set_flashdata('success', $message);
        redirect('admin/skill_management/all_skill/'.$data['skill_for']);
    }
