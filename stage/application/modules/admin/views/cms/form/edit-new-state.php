                    ...
                    <div class="page-content">
                        ...
                        <div class="card radius-15">
                            <div class="card-body">
                                ...
                                <?php } ?>
                                <hr/>
                                <form action="<?php echo base_url('admin/address/edit-new-state-succ'); ?>" method="post" onsubmit="return form_validation();">
                                    <input type="hidden" name="TRXTR" value="<?php echo $state_list[0]['id']; ?>">
                                    <div class="form-group">
                                        <label>Country:</label>
                                        <select class="form-control" id="country_id" name="country_id">
                                            <option value=""> Select Country </option>
                                            <?php foreach($country_list as $val){ ?>
                                                <option value="<?php echo $val['id']; ?>" <?php if($state_list[0]['country_id'] == $val['id']){ echo'selected'; } ?>> <?php echo $val['name']; ?> </option>
                                            <?php } ?>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label>State Name:</label>
                                        <input type="text" class="form-control" id="name" name="name" placeholder="Name" value="<?php echo $state_list[0]['name']; ?>">
                                    </div>
                                    <div class="form-group">
