...
class Cms_management extends MX_Controller {
    ...
    public function attribute_list(){
        ...
        $select = array('id','name','p_id','status');
        $con = array();
        $data['attribute_list'] = $this->admin_master_model->dynamic_get($select,'attribute',$con,'id','DESC');

        for ($i=0;$i<count($data['attribute_list']);$i++){ 

            $data['attribute_list'][$i]['total_attr']  = count($this->admin_master_model->dynamic_get($select,'attribute',array('p_id'=>$data['attribute_list'][$i]['id'])));

            if($data['attribute_list'][$i]['p_id'] > 0)
             $data['attribute_list'][$i]['attr_name']  = $this->admin_master_model->dynamic_get(array('name'),'attribute',array('id'=>$data['attribute_list'][$i]['p_id']));
            
        }
        $this->load->view('cms/list/attribute-list',$data);
    }

    public function fetch_attribute_list(){
        $this->method_chk($_SERVER['REQUEST_METHOD'], 'POST');
        $this->login_chk();
        $select = array('id','name','p_id','status');
        ...
        $attribute_list = $this->admin_master_model->dynamic_get($select,'attribute',$con,'id','DESC');
        if(count($attribute_list) > 0){
            $status = 1;
        }else{
             $status = 0;
        }
        $data = array('status'=>$status,'attribute'=>$attribute_list);
    ...
    public function add_edit_attribute(){
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                'p_id'=>$this->input->post('parent_attribute'),
                'name'=>$this->input->post('attribute_name'),
                'status'=>$this->input->post('attribute_name'),
                'created_by'=>get_admin_session_id(),
                'updated_by'=>get_admin_session_id(),
