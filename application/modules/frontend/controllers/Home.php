...
class Home extends MX_Controller {
    ...
    public function client_login_suc() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (count($user_det) > 0) {
                $this->session->set_flashdata('sucmsg', 'Signed in successfully.');
...
//    public function professional_registratio(){
//        $this->method_chk($_SERVER['REQUEST_METHOD'],'GET');
//        /////////////// User Role Fetch //////////////
//        $select = array('id', 'name');
//        $con = array('id >'=>2,'id <'=>5);
...
//    }
    ...
    public function professional_registration_success() {
        ...
        if (count($this->input->post('skills')) < 1) {
            $response = array('status' => 0, 'message' => 'select skill');
...
//            print_r($data);
    ...
    public function professional_registration_contact() {
        ...
        if ($company_page_det['pro_reg_1'] != 'Continue with Contact Info') {
            ...
            redirect('professional-registration');
            die;
        }
    ...
    public function professional_registration_contact_succ() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $response = array('status' => 1, 'message' => 'Contact info submitted');
            echo json_encode($response);
            die;
        }
    }
    ...
    public function professional_registration_bank() {
        ...
        if ($company_page_det['pro_reg_contact'] != 'Continue with Bank Account') {
            ...
            redirect('professional-registration-contact');
            die;
        }
    ...
    public function professional_registration_complete_old() {
        $this->method_chk($_SERVER['REQUEST_METHOD'], 'POST');
        $this->load->library('form_validation');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $user_detail_data['role_id'] = 3;
            $user_detail_data['name'] = $pro_reg_contact_2['fname'] . ' ' . $pro_reg_contact_2['lname'];
            $user_detail_data['email'] = $pro_reg_contact_2['email'];
            $user_detail_data['password'] = base64_encode($pro_reg_contact_2['password1']);
            $user_detail_data['std_code'] = $pro_reg_contact_2['std_code'];
            $user_detail_data['mobile_no'] = $pro_reg_contact_2['mobile_no'];
            ...
            $notification_array['type'] = 'Professional Registration';
            $notification_array['message'] = $user_detail_data['name'] . ' registered as a professional user.';
            ...
            $maildata = array();
            $maildata['to'] = $pro_reg_contact_2['email'];
            $maildata['verification_link'] = base_url() . 'professional-mail-verify/' . smart_encode($res);
            $maildata['dear_name'] = $user_detail_data['name'];
            $maildata['subject'] = 'Verify your Email ';
            $maildata['email_template'] = 'professional_registration_mail_varify';
            ...
            $professional_details['sub_work_field'] = isset($pro_reg_contact['sub_work_field']) ?$pro_reg_contact['sub_work_field'] : '' ;
            $professional_details['thumnail_image'] = $pro_reg_contact['company_logo'];
            ...
            if ($pro_reg_contact['other_skill']) {
                //$other_skill_arr = explode(',',$pro_reg_contact['other_skills']);
                $request_skill_arr = array();
                if (count($pro_reg_contact['other_skill']) > 0) {
                    foreach ($pro_reg_contact['other_skill'] as $val) {
                        $request_skill_arr['skill_for'] = 'professional';
                        $request_skill_arr['name'] = $val;
                        $request_skill_arr['status'] = '2';
                        $request_skill_arr['requested_by'] = $res;
                        $request_skill_arr['requested_time'] = date('Y-m-d H:i:s');
                        $res_skill_request = $this->client_master_model->dynamic_insert('skills', $request_skill_arr);
                        array_push($pro_reg_contact['skills'], $res_skill_request);
                    }
                }
            }
            /////////////////////// Other skill insert into requested skill end  //////////////////////
            /////////////////////// User Skill Insertion //////////////////////////
            foreach ($pro_reg_contact['skills'] as $val) {
                $professional_skill = array();
                $professional_skill['user_id'] = $res;
                $professional_skill['skill'] = $val;
                if ($val != 'other') {
                    $res5 = $this->client_master_model->dynamic_insert('users_skill', $professional_skill);
                }
            }

            $this->session->set_flashdata('sucmsg', 'Successfully registred. Go to your mailbox and verify it.');
            $response = array('status' => 1, 'message' => 'Successfully registred. Go to your mailbox and verify it.');
            echo json_encode($response);
            die;
        }
    }
    ...
    public function cli_logout() {
        ...
        $this->session->set_flashdata('sucmsg', 'Signed out successfully.');
        redirect('');
        die;
    }
    ...
    public function forget_password_success() {
        ...
        if ($insert_data['email'] == '') {
            ...
            if (count($customer_det) > 0) {
                $customer_det1 = $this->client_master_model->get_customer_status_name(array('id', 'name', 'email'), $insert_data['email']);
                if (count($customer_det1) > 0) {
                    $data['otp'] = '1' . rand(100, 999);
                    $data['subject'] = 'Your password reset instructions';
                    $data['email'] = $customer_det1[0]['email'];
                    $data['name'] = $customer_det1[0]['name'];
//                    $mail_status = $this->client_master_model->sent_mail_with_template('otp', $data);
                    $maildata = array();
                    $maildata['to'] = $customer_det1[0]['email'];
                    $maildata['otp'] = $data['otp'];
                    $maildata['dear_name'] = $customer_det1[0]['name'];
                    $maildata['subject'] = 'Your password reset instructions';
                    $maildata['email_template'] = 'forget_password';
                    $maildata['site_url'] = base_url();
                    ...
                    if ($result) {
                        ...
                        $this->session->set_flashdata('sucmsg', 'OTP Sent Successfully To Your Registered Mail ID!');
    ...
    public function forget_password_verify_otp_success() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (count($otp_det) > 0) {
                ...
                //echo'<pre>'; print_r($password_update);die;
                /////   update otp ////
...
////        echo 212121;
//        $maildata = array();
//        $maildata['to'] = 'krishna.kundu@inwebinfo.com';
//        $maildata['dear_name'] = 'test user';
//        $maildata['subject'] = 'Welcome to Benasmart';
//        $maildata['email_template'] = 'test_mail';
//        $maildata['site_url'] = base_url();
//        $result = send_mail($maildata);
//        echo $result;
//    }
...
//            $company_details['address1']= $pro_reg_contact['address1'];
//            $company_details['address2']= $pro_reg_contact['address2'];
//            $company_details['country_id']= $pro_reg_contact['country_id'];
//            $company_details['city_id']= $pro_reg_contact['city_id'];

            $res1_2 = $this->client_master_model->dynamic_insert('professional_company_det', array('user_id'=>$res));
            $res1_3 = $this->client_master_model->dynamic_insert('smart_professional_about_us', array('user_id'=>$res));
            /////////////////////// User Bank Details Insertion //////////////////////////
            $professional_bank_details = array();
            $professional_bank_details['userid'] = $res;
            $professional_bank_details['bank_country_id'] = $data['bank_country_id'];
